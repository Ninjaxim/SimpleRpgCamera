		...
		public override void OnInspectorGUI()
		{
			...
			else if(_toolbarSelection == 2)
			{
				...
				if(_self.allowMouseDrag)
				{
					...
					_self.mouseDragInvertHorizontal = EditorGUILayout.Toggle("Invert Horizontal", _self.mouseDragInvertHorizontal);
					_self.mouseDragInvertVertical = EditorGUILayout.Toggle("Invert Vertical", _self.mouseDragInvertVertical);
					...
					_self.mouseDragZtoY = EditorGUILayout.Toggle("Vertical Y", _self.mouseDragZtoY);

					if(_self.mouseDragZtoY)
					{
						EditorGUI.indentLevel++;
						_self.mouseDragLocalY = EditorGUILayout.Toggle("Local Space", _self.mouseDragLocalY);
						EditorGUI.indentLevel--;
					}
			...
			else if(_toolbarSelection == 3)
			{
				...
				if(_self.allowRotation)
				{
					...
					_self.limitAngleY = EditorGUILayout.Toggle("Limit Y Angle", _self.limitAngleY);

					if(_self.limitAngleY)
					{
						EditorGUI.indentLevel++;
						EditorGUILayout.LabelField("Y Angle Limit [Min: " + _self.minAngleY + " | Max: " + _self.maxAngleY + "]");
						EditorGUILayout.MinMaxSlider(ref _self.minAngleY, ref _self.maxAngleY, -180, 180);
			...
			else if(_toolbarSelection == 4)
			{
				...
				if(_self.allowZoom || _self.allowZoomKeys)
				{
					...
					_content = new GUIContent("Reset Key", "Key for resetting the zoom distance");
					_self.keyResetZoom = (KeyCode)EditorGUILayout.EnumPopup(_content, _self.keyResetZoom);
