...
namespace PhatRobit
{
	public class SimpleRpgCamera : MonoBehaviour
	{
		...
		public bool mouseDragInvertHorizontal = false;
		public bool mouseDragInvertVertical = false;
		public bool mouseDragBreaksLock = false;
		public MouseButton mouseDragButton = MouseButton.None;
		public bool mouseDragZtoY = false;                                  // Makes the vertical drag go up / down instead of forward / back
		public bool mouseDragLocalY = false;								// Changes vertical drag from global to local space
		...
		public bool limitAngleY = true;									// Limit the Y rotation angle
		public float minAngleX = -180;                                  // The minimum X rotation angle
		public float maxAngleX = 180;                                   // The maximum X rotation angle
		public float minAngleY = -90;                                   // The minimum Y rotation angle
		public float maxAngleY = 90;                                    // The maximum Y rotation angle
		...
		public KeyCode keyResetZoom = KeyCode.None;						// Reset the zoom distance to the default
		...
		private float _defaultZoomDistance = 0;
		...
		void Update()
		{
			...
			if(mouseLook && disableWhileUnlocked)
			{
#if UNITY_5
				cursorLock = Cursor.lockState == CursorLockMode.Locked;
				Debug.Log("locked via mouselook");
#else
			...
			float joystickX = useJoystick ? Input.GetAxis(joystickHorizontalAxis) : 0;
			float joystickY = useJoystick ? Input.GetAxis(joystickVerticalAxis) : 0;
					...
					allowTouch && Input.touchCount > 0))
			{
				...
				if(useJoystick)
				{
					inputX += joystickX * joystickSensitivity.x;
					inputY += joystickY * joystickSensitivity.y;
				}
				...
				_angle.y -= inputY * (invertRotationY ? -1 : 1);

				// Limit the Y rotation angle
				if(limitAngleY)
				{
					_angle.y = Mathf.Clamp(_angle.y, minAngleY, maxAngleY);
				}
			...
			if(_controllable && ((disableZoomOverGui && !mouseOverGui) || !disableZoomOverGui))
			{
				if(allowZoom || allowZoomKeys)
				{
					...
					else
					{
						...
						if(Input.GetKeyDown(keyResetZoom))
						{
							distance = _defaultZoomDistance;
						}
				...
				if(_controllable && (allowEdgeMovement || allowEdgeKeys || allowMouseDrag || allowMovementDoubleClick))
				{
					...
					else
					{
						...
						if(allowMouseDrag && Input.GetMouseButton((int)mouseDragButton))
						{
							...
							if(mouseDragZtoY)
							{
								scrollVelocity = new Vector3(Input.GetAxis(mouseHorizontalAxis) * sensitivity.x, Input.GetAxis(mouseVerticalAxis) * sensitivity.y, 0);

								if(mouseDragInvertHorizontal)
								{
									scrollVelocity.x = -scrollVelocity.x;
								}

								if(mouseDragInvertVertical)
								{
									scrollVelocity.y = -scrollVelocity.y;
								}
							}
							else
							{
								...
								if(mouseDragInvertVertical)
								{
									scrollVelocity.z = -scrollVelocity.z;
								}
							}
						}
					}
					...
					if(mouseDragZtoY && mouseDragLocalY && Input.GetMouseButton((int)mouseDragButton))
					{
						referentialShift = _t.rotation;
					}
				...
				if(target)
				{
					...
					if(lockToTarget || (!allowEdgeMovement && !allowEdgeKeys && !allowMouseDrag))
					{
						...
						if(softTracking && softTrackingRadius > 0)
						{
							...
							{
								focalPointTarget = Vector3.MoveTowards(_focalPoint.position, focalPointTarget, Time.deltaTime * (distanceToTarget / softTrackingRadius) * softTrackingSpeed);
							}
							else
							{
								focalPointTarget = _focalPoint.position;
							}
						}
		...
		void LateUpdate()
		{
			if(_focalPoint)
			{
				...
				Quaternion angleRotation = Quaternion.Euler(_angle.y, _angle.x, 0);
				Quaternion cameraRotation = (target && useTargetAxis ? target.rotation * angleRotation : angleRotation);
				
				Quaternion currentRotation = Quaternion.Slerp(_oldRotation, cameraRotation, Time.deltaTime * (_userInControl || _returnTimer < returnDelay ? rotationSmoothing : ((returnToOrigin || stayBehindTarget) ? returnSmoothing : rotationSmoothing)));
